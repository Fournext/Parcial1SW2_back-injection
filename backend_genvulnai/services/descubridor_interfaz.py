...
from backend_genvulnai.domain.constants import PALABRAS_CLAVE_IA
...
class DescubridorInterfazService:
    ...
    def descubrir_interfaz(cls, page: Page) -> ResultadoInterfaz:
        ...
        if not candidatos_input:
            logger.info("No se encontraron campos de entrada evidentes mediante heurística específica.")
            return ResultadoInterfaz(
        ...
        candidatos_input.sort(key=lambda c: c.puntuacion, reverse=True)
        return cls.construir_resultado_desde_candidato(page, candidatos_input[0], len(candidatos_input))
        ...
        for selector_base in cls.SELECTORES_INPUTS:
            try:
                ...
                for idx, elem in enumerate(elementos):
                    ...
                    score = 1.0
                    texto_completo = f"{placeholder} {aria_label} {name_attr} {id_attr}".lower()
        ...
        for selector_btn in cls.SELECTORES_BOTONES:
            try:
                ...
                for idx, btn in enumerate(botones):
                    ...
                    # Bonificación especial para verbos de generación/envío
                    if any(v in texto_combinado for v in ('generar', 'generate', 'enviar', 'send', 'submit', 'analizar')):
                        score += 3.0
